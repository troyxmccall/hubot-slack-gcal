...
module.exports = function(robot) {
	...
	robot.respond(/(which|what|are any|are there any) (.+)s (are )?(available|free|open)/i, function(msg) {
		robot.emit('google:authenticate', msg, function(err, oauth) {
			...
			if (group) {
				var req = googleapis.calendar('v3').freebusy.query({
					auth: oauth,
					resource: {
						items: _.map(_.keys(group), function(g) {
							return {
								id: g
							}
						}),
					...
					if (err) {
						msg.reply("Error getting calendar availability");
	...
	robot.respond(/(book|reserve) (me )?(.+) for (.+)/i, function(msg) {
		robot.emit('google:authenticate', msg, function(err, oauth) {
			gcal.getPrimaryCalendar(oauth, function(err, calendar) {
				...
				var room_name = msg.match[3].toLowerCase(),
					room;
				...
				googleapis.calendar('v3').events.quickAdd({
					...
					if (err || !event) {
						msg.reply("Error creating event");
						return console.log(err);
					}
					googleapis.calendar('v3').events.patch({
						auth: oauth,
						calendarId: calendar.id,
						eventId: event.id,
						resource: {
	...
	robot.respond(/invite (.*)/i, function(msg) {
		robot.emit('google:authenticate', msg, function(err, oauth) {
			...
			gcal.getPrimaryCalendar(oauth, function(err, calendar_o) {
				...
				googleapis.calendar('v3').events.get({
					auth: oauth,
					...
					googleapis.calendar('v3').events.patch({
						...
						eventId: event.id,
						resource: {
							attendees: _.union(emails, current_emails)
						}
					}, function(err, event) {
